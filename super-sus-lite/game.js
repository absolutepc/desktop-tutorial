(() => {
  ...
  function updateHud() {
    ...
    if (!human.alive) {
      ...
      els.hint.textContent = "E — сообщить о теле";
    } else if (nearTask?.type === "emergency") {
      els.hint.textContent = "Space / E — экстренное собрание";
    } else if (nearTask?.type === "task" && !human.isImpostor) {
      els.hint.textContent = "Удерживай E — выполнить задачу";
    } else if (human.isImpostor) {
      els.hint.textContent = human.killCd > 0
        ? `Убийство через ${human.killCd.toFixed(1)}с · Q рядом с жертвой`
        : "Q — убить ближайшего";
    } else {
  ...
  function updateHuman(dt) {
    ...
    if (keys.has("KeyE") || keys.has("Space")) {
      if (inter?.type === "body") {
        ...
        // also a bit of suspicion for reporter if impostor reports
